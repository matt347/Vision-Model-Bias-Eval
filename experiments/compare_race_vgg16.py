...
def get_race_id(race_value, race_names):
	...
	raise ValueError(
		f"Unable to map --train-race='{race_value}' to one of: {', '.join(race_names)}"
	)
...
def is_vgg_head_parameter(name):
	return name.startswith("classifier.6.")
...
def parse_args():
	parser = argparse.ArgumentParser(
		description="Train VGG16 gender classifier on one race and evaluate cross-race bias"
	)
...
def main():
	...
	if args.freeze_backbone:
		for name, param in model.named_parameters():
			if not is_vgg_head_parameter(name):
