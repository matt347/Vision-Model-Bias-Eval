...
def get_race_names(split_ds):
	race_feature = split_ds.features["race"]
	if hasattr(race_feature, "names") and race_feature.names is not None:
		return list(race_feature.names)
	unique_ids = sorted(set(split_ds["race"]))
...
def parse_args():
	parser = argparse.ArgumentParser(
		description="Train gender classifier on one race and evaluate cross-race bias"
	)
