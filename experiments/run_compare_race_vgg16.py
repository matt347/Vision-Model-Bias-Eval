...
def parse_args():
	parser = argparse.ArgumentParser(
		description="Run the race-specific VGG16 gender bias experiment once per race and aggregate results"
	)
	parser.add_argument("--script", type=str, default="experiments/compare_race_vgg16.py")
	...
	parser.add_argument("--output-dir", type=str, default="results/vgg16/race_comparison")
...
def format_detail_table(rows):
	headers = ["Train Race", "Eval Race", "Eval Group", "Samples", "Accuracy", "Gap vs In-Dist"]
	data_rows = []
	for row in rows:
		data_rows.append(
			[
...
def main():
			...
			detailed_rows.append(
				{
					"train_race": report["train_race"]["name"],
					"eval_race": row["race"],
					"eval_group": row["group"],
					"n_samples": row["n_samples"],
					"accuracy": row["accuracy"],
					"gap_vs_in_dist": row["gap_vs_in_dist"],
				}
			)
	...
	txt_path = output_root / "race_comparison_summary.txt"
	summary_lines = []
	summary_lines.append("=== Run-Level Comparison ===")
	summary_lines.append(format_table(aggregated_rows))
	summary_lines.append("")
	summary_lines.append("=== Per-Race Evaluation Comparison ===")
	summary_lines.append(format_detail_table(detailed_rows))
	summary_lines.append("")
	summary_lines.append(f"Aggregate JSON: {json_path}")
